...
<div class="create-page">
    <div class="form-card"
         x-data="{
            preview: @json($meal->image_url ?? null),
            calories: @json($meal->calories ?? ''),
            protein: @json($meal->protein ?? ''),
            carbs: @json($meal->carbs ?? ''),
            fat: @json($meal->fat ?? ''),
        ...
        <div class="form-title">Edit a meal</div>
        <div>
            
        </div>
        <div class="form-subtitle">Track what you ate and your macros.</div>

        <form method="POST" action="{{ route('meals.update', ['id' => $meal->id]) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="upload-zone" :class="{ 'has-preview': preview }"
                ...
                @if ($meal->image_path)
                <img class="upload-preview-edit" src="{{ Storage::url($meal->image_path) }}" />
                <p class="upload-preview-overlay">Current image — upload a new one to replace it</p>
                @endif

                <input type="file" name="image" accept="image/*" x-ref="fileInput" value="{{ $meal?->image_path }}" @change="setImage($event)">
            </div>
            ...
            {{-- Date --}}
            <div class="field">
                <label for="eaten_at">Date</label>
                <input type="date" id="eaten_at" name="eaten_at"
                       value="{{ $meal?->eaten_at }}" 
