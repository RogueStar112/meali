     ...
     x-data="{
        ...
        <form method="POST" action="{{ route('meals.store') }}" enctype="multipart/form-data">
            ...
            <div class="upload-zone" :class="{ 'has-preview': preview }"
                ...
                <input type="file" name="image" id="image-file" accept="image/*" x-ref="fileInput" @change="setImage($event)">
    ...
    $('#api-fill').on('click', function(e) {
        ...
        fetch(`/meal/search/${encodeURIComponent(name)}`, { cache: 'no-store' })
            ...
            .then(data => {
                ...
                $('#calories').val(Math.round(data['calories'], 0) ?? 0);
                $('#fat').val(Math.round(data['fat'], 1) ?? 0);
                $('#carbs').val(Math.round(data['carbs'], 1) ?? 0);
                $('#protein').val(Math.round(data['protein'], 1) ?? 0) ;
